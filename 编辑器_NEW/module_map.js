     ...
     setup() {
         ...
         watch(() => mapCurrentScene.value?.points, (points) => {
             ...
             points.forEach(async (p) => {
                 if (p.charId && state.db.characters) {
                     const c = state.db.characters.find(x => x.id == p.charId);
         ...
         const createNewScene = () => {
             ...
             if (!state.db.mapTree[folder]) {
                 state.db.mapTree[folder] = [];
             }
         ...
         const startDragPoint = (e, idx, p) => {
             local.curPointIdx = idx;
             local.activeTab = 'pt-base';
             local.isDraggingPoint = true;
         ...
         const isNpcMode = (p) => p.charId && state.db.characters && state.db.characters.some(c => c.id == p.charId);
         const getNpcChar = (p) => state.db.characters ? state.db.characters.find(c => c.id == p.charId) : null;
         
         const getPointStyle = (p) => {
             // 兼容并热升级旧的绝对像素数据 -> 百分比数据
             ...
             if (isNpcMode(p)) {
                 ...
                 if (c && c.defaultPortrait) {
                     ...
                     style.color = '#fff'; style.fontSize = '24px';
                 }
             }
             return style;
         };
         ...
         // -----------------------------------------------------------------------------
         // 5. 宏指令编译器逻辑 (Macro Compiler)
         // -----------------------------------------------------------------------------
         const openMacroGenerator = (actIndex) => {
             macroModal.targetActIndex = actIndex;
             macroModal.activeActionId = '';
             macroModal.fieldValues = {};
             macroModal.condId = '';
             macroModal.condValues = {};
         ...
         return {
             state, local, mapCurrentScene, mapCurrentPoint, dollDb,
             macroModal, schema,
             createNewScene, loadScene, addPoint, startDragPoint,
             isNpcMode, getNpcChar, getPointStyle, copyToClipboard,
             openMacroGenerator, insertMacro
         };
     },
                              ...
                              <div id="explore-layer" style="position: absolute; width: 100%; height: 100%; z-index: 10;" @dblclick="addPoint">
                                  <div v-for="(p, i) in mapCurrentScene?.points || []" :key="p.id" 
                                      ...
                                      <template v-if="isNpcMode(p)">
                                          <div style="position:absolute; top:-35px; background:rgba(0,0,0,0.8); padding:2px 8px; border-radius:var(--radius-sm); font-size:12px; white-space:nowrap; border:1px solid var(--primary); z-index: 100;">
                                              <span v-if="getNpcChar(p).title" class="text-secondary">[{{getNpcChar(p).title}}]</span> 
                                              <b class="text-primary">{{getNpcChar(p).name}}</b>
                                          </div>
                                      </template>
                                      <template v-else>
                                          <div style="font-size: 24px; text-shadow: 0 2px 4px #000; filter: drop-shadow(0 0 5px rgba(0, 242, 254, 0.8));">
                                              {{ p.icon === 'search' ? '🔍' : (p.icon === 'door' ? '🚪' : (p.icon === 'talk' ? '💬' : '✋')) }}
                                          </div>
                                          <div style="position:absolute; top:-30px; background:rgba(0,0,0,0.8); padding:2px 8px; border-radius:var(--radius-sm); font-size:12px; white-space:nowrap; color:#fff;">
                                              {{ p.name || '未命名' }}
                                          </div>
                                      </template>
                                  </div>
                      ...
                      <div v-show="local.activeTab==='pt-base'" class="p-10">
                          ...
                          <div class="card p-10 flex-column" style="border-color: var(--primary);">
                              ...
                              <select class="select" v-model="mapCurrentPoint.charId" style="width: 100%;">
                                  ...
                                  <option v-for="c in state.db.characters" :key="c.id" :value="c.id">{{c.name}}</option>
                              </select>
                          </div>
                      </div>
